...
I decided to use a Queue for my representation because I pretty much modeled my component off of a Queue to begin with so it's definitely the easiest representation to use. For my playlist I made it based on FIFO, so you remove songs from the front and add them to the back, which makes Queue a good representation to use since it is essentially the same thing. I decided to use the OSU Queue component because I have already designed everything else based on OSU standards so I figured I would just stick with that.
